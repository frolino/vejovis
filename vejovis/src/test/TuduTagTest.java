...
public class TuduTagTest {
	...
	static final String INJECTIONSFOLDER = "tuduTagInjections/";
	...
	public static void main(String[] args) {
		...
		File file = new File("/tuduInjections/results_tag.txt");
			...
			try {
				...
				for (String injection : injectionStrs) {
					...
					while (st.hasMoreTokens()) {
						...
						else if (nextLine.startsWith("INDEX CALL")) {
							srcLine_indexCall = nextLine.substring(12);
						}
						else if (nextLine.startsWith("INDEX LINE")) {
							lineNo_indexCall = Integer.parseInt(nextLine.substring(12));
						}
						else if (nextLine.startsWith("INDEX FUNC")) {
							funcName_indexCall = nextLine.substring(12);
						}
						else if (nextLine.startsWith("INDEX EXPR")) {
							indexExpr = nextLine.substring(12);
						}
					}
					
					if (gotOutput1 && gotOutput2 && gotOutput3 && gotOutput4 && gotOutput5 && gotOutput6 && gotOutput7) {
						//Set up index call (if applicable)
						...
						InjectionInput inj = new InjectionInput(output1, output2, output3, output4, output5, output6, output7, idxCall);
						if (output8 == 1) {
							inj.setType();
						}
						injList.add(inj);
		...
		for (int i = injStart; i < injList.size(); i++) {
			...
			try {
				...
				FixClassSelector fcs = new FixClassSelector(strSet, extractor.getErroneousID(), dda, states, currentState, injList.get(i).getIndexCall(), extractor.getGebtnParam(), tags); //create module that gets erroneous ID after
	...
	private static void test1() {
		...
		tuduExclude.add(".*dwr.*");
		tuduExclude.add(".*scriptaculous.*");
		tuduExclude.add(".*calendar.*");
